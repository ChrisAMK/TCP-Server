...
export default {
    saveRig8: (uData) => {
        let rawCalculations = {
           ...
           intakeManifoldTemp: uData[83] * 256 + uData[82]                                          || invalidValue,
           turboRpm: uData[85] * 256 + uData[84]                                                    || invalidValue,
           electricalPotential: uData[87] * 256 + uData[86]                                         || invalidValue,
           engineOilLevel: uData[89] * 256 + uData[88]                                                          || invalidValue,
           timestamp: parseInt((((uData[91] << 8) + uData[90]).toString() + ((((uData[95] << 8 | uData[94]) << 8 | uData[93]) << 8) + uData[92]).toString()))   || invalidValue,
        }
        ...
        let postCalculations = {
            ...
            engineTorque: preCalculations.engineTorque                                        || invalidValue,
            intercoolerTemp: preCalculations.intercoolerTemp                                  || invalidValue,
            totalFuelUsed: preCalculations.totalFuelUsed                                      || invalidValue,
            intakeManifoldTemp: preCalculations.intakeManifoldTemp                            || invalidValue,
            turboRpm: preCalculations.turboRpm                                                || invalidValue,
            electricalPotential: preCalculations.electricalPotential                          || invalidValue,
            engineOilLevel: preCalculations.engineOilLevel                                    || invalidValue,
        }
        ...
        MKY08RAW.create({
            ...
            engineTorque: rawCalculations.engineTorque,
            intercoolerTemp: rawCalculations.intercoolerTemp,
            totalFuelUsed: rawCalculations.totalFuelUsed,
            intakeManifoldTemp: rawCalculations.intakeManifoldTemp,
            turboRpm: rawCalculations.turboRpm,
            electricalPotential: rawCalculations.electricalPotential,
            engineOilLevel: rawCalculations.engineOilLevel,
            year: timeConverter(rawCalculations.timestamp).year,
            month: timeConverter(rawCalculations.timestamp).month,
            date: timeConverter(rawCalculations.timestamp).date,
            hour: timeConverter(rawCalculations.timestamp).hour,
